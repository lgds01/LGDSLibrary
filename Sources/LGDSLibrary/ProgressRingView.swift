...
struct ProgressRingView: View {
    ...
    var gradient = Gradient(colors: [.green,.green])
    ...
    var body: some View {
        ZStack {
            Circle()
                .stroke(Color(.systemGray), lineWidth: thickness)
            ...
            RingTip(progress: progress, startAngle: startAngle, ringRadius: radius)
                ...
                .foregroundColor(progress > 0.96 ? gradient.stops[1].color : .clear)
                .shadow(color: progress > 0.96 ? .black.opacity(0.15) : .clear, radius: 2 , x: ringTipShadowOffset.x, y: ringTipShadowOffset.y)
        }
...
struct RingShape: Shape {
    ...
    func path(in rect: CGRect) -> Path {
        ...
        path.addArc(center: CGPoint(x: rect.width / 2.0, y: rect.height / 2.0), radius: rect.width / 2.0, startAngle: .degrees(startAngle), endAngle: .degrees(startAngle + progress * 360), clockwise: false)
...
struct ProgressRingView_Previews: PreviewProvider {
    static var previews: some View {
        ProgressRingView(progress: .constant(0.8))
    }
}
